...
serve(async (req) => {
  ...
  try {
    ...
    // Step 1: List GA4 properties to find one matching this domain
    console.log(`[ga4] Looking up properties for domain: ${domain}`);
    ...
    // Find property matching this domain
    ...
    for (const account of summaries) {
      for (const prop of account.propertySummaries || []) {
        // Property names often contain the domain
    ...
    // If no match by name, try data streams
    if (!propertyId) {
      for (const account of summaries) {
        for (const prop of account.propertySummaries || []) {
    ...
    if (!propertyId) {
      return new Response(JSON.stringify({
        success: true,
        found: false,
        message: `No GA4 property found matching "${domain}". Available properties: ${summaries.flatMap((a: any) => (a.propertySummaries || []).map((p: any) => p.displayName)).join(', ')}`,
    ...
    console.log(`[ga4] Found property: ${propertyName} (${propertyId})`);

    // Step 2: Run GA4 Data API report — last 30 days overview
    ...
    // Parallel requests: overview metrics + top pages + traffic sources + daily trend
