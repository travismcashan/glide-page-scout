...
import {
  ...
  MessageSquare, PhoneCall, Calendar, StickyNote, CheckSquare
} from 'lucide-react';
...
export default function CompanyDetailPage() {
  ...
  const growth6mo = enrichment.organizationHeadcountGrowth6mo;
  const growth12mo = enrichment.organizationHeadcountGrowth12mo;
            ...
            <div className="flex flex-wrap gap-x-5 gap-y-1.5 text-sm text-muted-foreground">
              ...
              {company.employee_count && (
                <span className="flex items-center gap-1">
                  <Users className="h-3.5 w-3.5" /> {company.employee_count} employees
                </span>
              )}
              {company.annual_revenue && (
                <span className="flex items-center gap-1">
                  <TrendingUp className="h-3.5 w-3.5" /> {company.annual_revenue}
                </span>
              )}
              {company.location && (
                <span className="flex items-center gap-1">
          ...
          <div className="lg:col-span-2">
                      ...
                      <div key={contact.id} className="flex items-start gap-3 p-3 rounded-lg border border-border/50 hover:border-border transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {contact.seniority && (
                              <Badge variant="secondary" className={`text-[10px] py-0 ${SENIORITY_COLORS[contact.seniority] || ''}`}>
                          ...
                          <div className="flex flex-wrap gap-x-3 gap-y-1 mt-1.5 text-xs text-muted-foreground">
                            ...
                            {contact.linkedin_url && (
                              <a href={contact.linkedin_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-foreground">
                      ...
                      <div key={deal.id} className="flex items-start gap-3 p-3 rounded-lg border border-border/50">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant="outline" className={deal.status === 'won' ? 'bg-green-500/15 text-green-400' : deal.status === 'lost' ? 'bg-red-500/15 text-red-400' : 'bg-blue-500/15 text-blue-400'}>
                              {deal.status}
                            </Badge>
                          </div>
            ...
            {engagements.length > 0 && (
              <Card className="mt-6">
                <CardHeader className="pb-3">
                  <CardTitle className="text-base flex items-center gap-2">
                    <MessageSquare className="h-4 w-4" /> Recent Activity ({engagements.length})
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
                    {engagements.map(eng => {
                      ...
                      }[eng.engagement_type] || <MessageSquare className="h-3.5 w-3.5" />;
                      ...
                      return (
                        <div key={eng.id} className="flex items-start gap-2.5 p-2.5 rounded-lg hover:bg-accent/5 transition-colors">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {eng.subject && eng.subject !== eng.engagement_type.charAt(0).toUpperCase() + eng.engagement_type.slice(1) && (
                              <p className="text-xs text-muted-foreground truncate mt-0.5">{eng.subject}</p>
                            )}
                            {eng.body_preview && (
                              <p className="text-xs text-muted-foreground/60 line-clamp-2 mt-0.5">{eng.body_preview}</p>
                            )}
                          </div>
                        </div>
                      );
            ...
            {technologies.length > 0 && (
              <Card className="mt-6">
                <CardHeader className="pb-3">
                  <CardTitle className="text-base">Technologies ({technologies.length})</CardTitle>
                </CardHeader>
                <CardContent>
                ...
                {sites.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {sites.map(site => (
                      <button
                        key={site.id}
                        onClick={() => navigate(buildSitePath(site.domain, site.created_at, 'raw-data'))}
                        className="w-full text-left p-3 rounded-lg border border-border/50 hover:border-border hover:bg-accent/5 transition-all group flex items-center gap-3"
                      >
                        <Globe className="h-4 w-4 text-muted-foreground shrink-0" />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium truncate">{site.domain}</p>
                          <p className="text-xs text-muted-foreground">
                            {format(new Date(site.created_at), 'MMM d, yyyy')} &middot; {site.status}
                          </p>
                        </div>
