...
export function GridView({ documents, onDelete, groupBy }: Props) {
        ...
        {groups.map(([label, docs]) => (
          <div key={label}>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
              {docs.map(doc => <GridItem key={doc.id} doc={doc} onDelete={onDelete} />)}
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2 px-1">
      {documents.map(doc => <GridItem key={doc.id} doc={doc} onDelete={onDelete} />)}
    </div>
  );
}
...
function GridItem({ doc, onDelete }: { doc: KnowledgeDocument; onDelete: (id: string, name: string) => void }) {
  ...
  return (
    <div className="group relative flex flex-col items-center gap-1.5 p-3 rounded-lg hover:bg-accent/50 transition-colors text-center">
      <button
