...
function UserBubbleWrapper({ content, attachmentNames, onEdit, disabled }: { content: string; attachmentNames?: string[]; onEdit: (newText: string) => void; disabled?: boolean }) {
  ...
  return (
    <div className="group relative max-w-[85%]">
      ...
      <div className="bg-muted/80 text-foreground rounded-lg rounded-tr-none px-4 py-3 text-[15px] border border-border">
...
export function KnowledgeChatCard({ session, pages, selectedModel, reasoning, onModelChange, onReasoningChange, onDocumentsChanged }: Props) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] items-center">
      {/* Messages area */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-4 px-1 pb-0 w-full max-w-3xl mx-auto">
      ...
      {/* Input area */}
      <div
        className={`mb-2 mt-0 rounded-2xl bg-card border-2 shadow-lg px-4 py-3 transition-colors w-full max-w-3xl mx-auto ${isDragging ? 'border-primary bg-primary/5' : 'border-border'}`}
        onDragEnter={(e) => {
          e.preventDefault();
          e.stopPropagation();
